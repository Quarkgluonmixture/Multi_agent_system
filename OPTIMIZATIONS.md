...
未做的优化项。每条都包含背景 + 具体方案，方便以后单独挑出来做。
...
**现状**：R1 三家有一家失败 → 整个 pipeline throw。
...
**现状**：某家在 submit 阶段 transient 失败（network、tab 被关、navigation 抢资源）→ 直接标 failed，不重试。

**方案**：在 `runProviderJob` 包一层 retry，最多 1 次重试，间隔 3s。已经成功 submit 的不重试（避免重复扣 token）。

### 6. ChatGPT `?temporary-chat=true` 隐式依赖

**现状**：硬编码 URL 参数。如果 OpenAI 改变这个机制，会静默退化到普通对话（污染历史）。

**方案**：进入 ChatGPT 后探测临时对话标志（页面顶部"temporary"文字 / specific class），不在则告警。

### 7. SW 重启时 ourTabs 全丢

**现状**：MV3 service worker 空闲会被回收，ourTabs Map 在内存里，重启就空。下次 run 会创建新 tab，老的还留着。
...
### 10. Round-level 持久化 + 查看历史

**现状**：每次 run 完，结果只在 side panel 内存。关掉就没了。

**方案**：用 `chrome.storage.local` 存最近 N 次 run 的完整 RunState（user prompt、各 round 输出、final）。side panel 加"历史"切换。

### 11. 流式渲染输出到 side panel

**现状**：每个 provider done 才一次性把 output 塞进 card。
...
**现状**：只支持 ChatGPT / Gemini / DeepSeek。

**候选**（按优先级）：
- Claude (claude.ai) — 用户当前因 token 节省排除，但以后可能加回
- Kimi (kimi.com) — 长 context 200k，对长文输入有优势
